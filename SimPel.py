...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QMessageBox,
                             QVBoxLayout, QFileDialog, QSplashScreen, QDialog)
...
from Calculation_PELDOR_TT import (calculate_time_trace, add_background,
                                   add_noise)
...
configs = {"r_min": 1.2, "r_max": 8, "sigma_max": 1.1, "sigma_min": 0.0,
           "axeslabelfontsize": 9, "fontsize": 10, "xmargin": 0.0,
           "ymargin": 0.1, "xsize": 4, "ysize": 3, "points": 881,
           "linewidth": 1.5, "t_min": -200}

configs_str = {"ylabel_time_trace": r'$Intensity$',
               "xlabel_time_trace": r"$time\ \mathrm{/ \ ns}$",
               "ylabel_dd": r'$Normalised\ \ Intensity$',
               "xlabel_dd": r'$r\ \mathrm{/ \ nm}$', "autosave": "True",
               "figure_format": 'png,pdf', "yticks_time_trace": "True",
               "yticks_dd": "False", "title_time_trace": "True",
               "title_dd": "True", "colorL": "blue", "colorBG": "red",
               "showuserTT": "True", "ColorUser": "grey"}
...
configs_lim_max = {"r_max": 16, "sigma_max": 1.1, "xmargin": 0.5,
                   "ymargin": 0.8, "xsize": 100, "ysize": 100,
                   "linewidth": 10, "t_min": 0}
...
class SimPelDesign(QMainWindow, SimPel_Layout.Ui_SimPel2018):
    def __init__(self):
        ...
        self.Distance1edit.textChanged.connect(lambda: self.read_dist(
                                               self.Distance1edit.text(), 0))
        self.Distance2edit.textChanged.connect(lambda: self.read_dist(
                                               self.Distance2edit.text(), 1))
        self.Distance3edit.textChanged.connect(lambda: self.read_dist(
                                               self.Distance3edit.text(), 2))
        self.Distance4edit.textChanged.connect(lambda: self.read_dist(
                                               self.Distance4edit.text(), 3))
        self.Distance5edit.textChanged.connect(lambda: self.read_dist(
                                               self.Distance5edit.text(), 4))
        self.Coeff1edit.textChanged.connect(lambda: self.read_coeff(
                                            self.Coeff1edit.text(), 0))
        self.Coeff2edit.textChanged.connect(lambda: self.read_coeff(
                                            self.Coeff2edit.text(), 1))
        self.Coeff3edit.textChanged.connect(lambda: self.read_coeff(
                                            self.Coeff3edit.text(), 2))
        self.Coeff4edit.textChanged.connect(lambda: self.read_coeff(
                                            self.Coeff4edit.text(), 3))
        self.Coeff5edit.textChanged.connect(lambda: self.read_coeff(
                                            self.Coeff5edit.text(), 4))
        self.Standard1.textChanged.connect(lambda: self.read_sigmas(
                                            self.Standard1.text(), 0))
        self.Standard2.textChanged.connect(lambda: self.read_sigmas(
                                           self.Standard2.text(), 1))
        self.Standard3.textChanged.connect(lambda: self.read_sigmas(
                                           self.Standard3.text(), 2))
        self.Standard4.textChanged.connect(lambda: self.read_sigmas(
                                           self.Standard4.text(), 3))
        self.Standard5.textChanged.connect(lambda: self.read_sigmas(
                                           self.Standard5.text(), 4))
        ...
        self.EnableBgButton.toggled.connect(lambda: self.btn1state(
                                            self.EnableBgButton))
        self.ShowBGButton.toggled.connect(lambda: self.btn2state(
                                          self.ShowBGButton))
        self.Sigmaedit.textChanged.connect(self.read_sigma)
        self.Universal_standard_dev_button.toggled.connect(lambda:
                  self.use_universal_sigma(self.Universal_standard_dev_button))
        self.EnableNoiseButton.toggled.connect(lambda: self.btn3state(
                                               self.EnableNoiseButton))
        self.SNR_edit.textChanged.connect(self.read_SNR)
        self.Checkbox_superadaptive.stateChanged.connect(lambda:
                  self.use_superadaptive_Gaussian(self.Checkbox_superadaptive))
        self.ButtonFullyAdaptive.toggled.connect(lambda: self.btn5state(
                                                 self.ButtonFullyAdaptive))
        self.fresnelbutton.toggled.connect(lambda: self.btn7state(
                                           self.fresnelbutton))
        self.User_defined_distr.toggled.connect(lambda: self.Userdef_Dist(
                                                self.User_defined_distr))
        ...
        if name.endswith('.txt') or name.endswith('.TXT'):
            try:
                ...
                if min(k1) <= 0 or max(k1) > 10 or min(k2) < 0:
                    str_tm = ("Weird distance distribution. Simulation won't" +
                              " be carried out!")
                    self.War3 = QMessageBox.warning(self, "Corrupted data",
                                                    str_tm, QMessageBox.Ok)
                else:
                    self.r_usr = k1
                    self.Pr_usr = k2
                    self.Label_user_def_dist.setText(str(self.filename))
            except:
                str_tm = "Something went wrong! Data could not be loaded. "
        ...
        if name.endswith('.txt') or name.endswith('.TXT'):
            try:
                ...
                if ind == -1:
                    ...
                    self.filename = name[ind+1:]
                if min(k1) <= -500 or max(k1) > 1e8:
                    str_tm = "Weird PELDOR trace. Cannot be plotted!"
                    self.War3 = QMessageBox.warning(self, "Corrupted data",
                                                    str_tm, QMessageBox.Ok)
                else:
                    self.time_usr = k1
            ...
            try:
                ...
                (self.TT_usr, self.imag,
                 self.time_usr) = read_Elexsys_File(Input, name)
                self.make_calculation()
                self.TT_userdef = True
                try:
                    self.make_calculation()
    ...
    def Userdef_Dist(self, integr):
        if self.User_defined_distr.isChecked():
            if self.Pr_usr is None:
                str_tm = "No distance distribution loaded yet!"
    ...
    def make_calculation(self):
        ...
        if self.User_defined_distr.isChecked():
            ...
            self.userdef = False
        if self.Universal_standard_dev_button.isChecked():
        ...
        # Simulation kernel functions are called for PELDOR time traces
        self.Pr, self.r = calculate_distance_distr(self.inputvalues,
                                                   self.sigmas, self.configs)
        self.JarasprogressBar.setValue(0)
        QApplication.processEvents()
        self.time = calculate_time(self.timescale, self.stepsize,
                                   self.configs["t_min"])
        (self.spectrum, self.Fourier, self.Frequency_region,
         self.warning) = calculate_time_trace(self.inputvalues, self.time,
                                              self.moddepth, self.integrator,
                                              self.superadap,
                                              self.JarasprogressBar,
                                              self.sigmas, self.userdef,
                                              self.r_usr, self.Pr_usr)
        self.JarasprogressBar.setValue(0)
        ...
        if hasattr(self, 'figure4') or hasattr(self, 'figure5'):
            self.detach_plot_TT()
        if self.warning:
            str_tm = ("The given distance distribution has contributions in" +
                      " the negative distance domain.\n\nYou should reduce" +
                      " the standard deviation or increase the distance!")
            self.War = QMessageBox.warning(self, "Negative distances.", str_tm,
                                           QMessageBox.Ok)
            self.warning = False
    ...
    def make_background(self):
        if self.bg_on == 1:
            self.spc_bg_n, self.bg_function = add_background(self.spc_n,
                                                             self.time,
                                                             self.bg_dim,
                                                             self.bg_decay,
                                                             self.moddepth)
    ...
    def make_noise(self):
        if self.noise_on == 1:
            ...
            self.spc_n, self.noisefunction = add_noise(self.spectrum,
                                                       self.noislevel)
        else:
    ...
    def read_dist(self, text, n):
        try:
            if (float(text) <= self.configs["r_max"] and
               float(text) >= self.configs["r_min"]):
    ...
    def read_sigmas(self, text, n):
        try:
            if (float(text) <= configs["sigma_max"] and
               float(text) > configs["sigma_min"]):
    ...
    def use_superadaptive_Gaussian(self, adaptiveon):
        if (adaptiveon.isChecked() and not self.integrator == 2):
            self.superadap = True
        elif(adaptiveon.isChecked() and self.integrator == 2):
            self.superadap = False
            self.Checkbox_superadaptive.setCheckState(False)
        else:
            self.superadap = False
    ...
    def save_file(self):
        ...
        save_name, name_tmp = dialog.getSaveFileName(self, 'Save File', None,
                                                     "*.txt (*.txt);;" +
                                                     "*dat (*.dat)")
        if not save_name:
            return False
    ...
    def plot_TT(self):
        ...
        self.ax.axis('on')
        self.ax.set_xlabel(self.configs_str["xlabel_time_trace"],
                           size=self.configs["fontsize"])
        self.ax.set_ylabel(self.configs_str["ylabel_time_trace"],
                           size=self.configs["fontsize"])
        if self.TT_userdef and eval(self.configs_str["showuserTT"]):
            if self.EnableBgButton.isChecked():
                self.TT_usr_tmp = self.TT_usr/max(abs(self.TT_usr))
            else:
                self.TT_usr_tmp = self.TT_usr
            self.ax.plot(self.time_usr, self.TT_usr_tmp,
                         color=self.configs_str["ColorUser"].replace(" ", ""),
                         linewidth=self.configs["linewidth"])
        if eval(self.configs_str["title_time_trace"]):
            self.ax.set_title('PELDOR/DEER trace',
                              size=self.configs["fontsize"])
        for tick in self.ax.xaxis.get_major_ticks():
                tick.label.set_fontsize(self.configs["axeslabelfontsize"])
        for tick in self.ax.yaxis.get_major_ticks():
                tick.label.set_fontsize(self.configs["axeslabelfontsize"])
        if not eval(self.configs_str["yticks_time_trace"]):
            self.ax.set_yticks([])
        if self.EnableBgButton.isChecked():
            spc = self.spc_bg_n/max(abs(self.spc_bg_n))
            self.ax.plot(self.time, spc,
                         color=self.configs_str["colorL"].replace(" ", ""),
                         linewidth=self.configs["linewidth"])
            if self.showbg == True:
                self.ax.plot(self.time_bg,
                             self.bg_function/max(abs(self.spc_bg_n)),
                             color=self.configs_str["colorBG"].replace(" ", ""),
                             linewidth=self.configs["linewidth"])
        else:
        ...
        self.figure.tight_layout()
        self.figure.subplots_adjust(left=0.18, bottom=0.18, right=0.96,
                                    top=0.92)
        self.canvas.draw()
        return
    ...
    def detach_plot_TT(self):
        ...
        if not hasattr(self, 'figure4'):
            ...
            self.ax4.axis('on')
            self.ax4.set_xlabel(self.configs_str["xlabel_time_trace"],
                                size=self.configs["fontsize"])
            self.ax4.set_ylabel(self.configs_str["ylabel_time_trace"],
                                size=self.configs["fontsize"])
            if self.TT_userdef and eval(self.configs_str["showuserTT"]):
                if self.EnableBgButton.isChecked():
                ...
                self.ax.plot(self.time_usr, self.TT_usr,
                             color=self.configs_str["ColorUser"].replace(" ", ""),
                             linewidth=self.configs["linewidth"])
            ...
            if self.EnableBgButton.isChecked():
                spc = self.spc_bg_n/max(abs(self.spc_bg_n))
                self.ax4.plot(self.time, spc,
                              color=self.configs_str["colorL"].replace(" ", ""),
                              linewidth=self.configs["linewidth"])
                if self.showbg:
                    self.ax4.plot(self.time_bg,
                                  self.bg_function/max(abs(self.spc_bg_n)),
                                  color=self.configs_str["colorBG"].replace(" ", ""),
                                  linewidth=self.configs["linewidth"])
            else:
                ...
                if self.showbg:
                    self.ax4.plot(self.time_bg, self.bg_function,
                                  color=self.configs_str["colorBG"].replace(" ", ""),
                                  linewidth=self.configs["linewidth"])
            self.ax4.margins(x=self.configs["xmargin"],
                             y=self.configs["ymargin"])
            self.figure4.tight_layout()
            self.figure.subplots_adjust(left=0.2, bottom=0.18, right=0.96,
                                        top=0.92)
            self.toolbar2 = Navi(self.canvas4, self.canvas4)
            self.canvas4.draw()
            self.canvas4.show()
        if not hasattr(self, 'figure5'):
            ...
            self.ax5.set_xlabel(self.configs_str["xlabel_dd"],
                                size=self.configs["fontsize"])
            self.ax5.set_ylabel(self.configs_str["ylabel_dd"],
                                size=self.configs["fontsize"])
            ...
            for tick in self.ax5.yaxis.get_major_ticks():
                    tick.label.set_fontsize(self.configs["axeslabelfontsize"])
            if self.userdef:
                self.ax5.plot(self.r_usr, self.Pr_usr/max(self.Pr_usr),
                              color=self.configs_str["colorL"].replace(" ", ""),
                              linewidth=self.configs["linewidth"])
            else:
                self.ax5.plot(self.r, self.Pr/max(self.Pr),
                              color=self.configs_str["colorL"].replace(" ", ""),
                              linewidth=self.configs["linewidth"])
            self.ax5.margins(x=self.configs["xmargin"],
                             y=self.configs["ymargin"])
            self.figure5.tight_layout()
            self.figure5.subplots_adjust(left=0.18, bottom=0.12, right=0.92,
                                         top=0.92)
            self.toolbar3 = Navi(self.canvas5, self.canvas5)
            self.canvas5.draw()
            self.canvas5.show()
        return
    ...
    def plot_DD(self):
        ...
        self.ax2.axis('on')
        self.ax2.set_xlabel(self.configs_str["xlabel_dd"],
                            size=self.configs["fontsize"])
        self.ax2.set_ylabel(self.configs_str["ylabel_dd"],
                            size=self.configs["fontsize"])
        self.ax2.set_ylim([-0.01, 1.06])
        if eval(self.configs_str["title_dd"]):
            self.ax2.set_title('Distance distribution',
                               size=self.configs["fontsize"])
        if not eval(self.configs_str["yticks_dd"]):
            self.ax2.set_yticks([])
        for tick in self.ax2.xaxis.get_major_ticks():
                tick.label.set_fontsize(self.configs["axeslabelfontsize"])
        for tick in self.ax2.yaxis.get_major_ticks():
        ...
        if not eval(self.configs_str["yticks_dd"]):
            self.ax2.set_yticks([])
        if self.userdef:
            self.ax2.plot(self.r_usr, self.Pr_usr/max(self.Pr_usr),
                          color=self.configs_str["colorL"].replace(" ", ""),
                          linewidth=self.configs["linewidth"])
        else:
            self.ax2.plot(self.r, self.Pr/max(self.Pr),
                          color=self.configs_str["colorL"].replace(" ", ""),
                          linewidth=self.configs["linewidth"])
        self.ax2.margins(x=self.configs["xmargin"], y=self.configs["ymargin"])
        self.figure2.tight_layout()
        self.figure2.subplots_adjust(left=0.16, bottom=0.18, right=0.98,
                                     top=0.92)
        self.canvas2.draw()
        return
    ...
    def plot_FT(self):
        ...
        self.ax3.set_ylabel('$Normalised\ \ Intensity$',
                            size=self.configs["fontsize"])
        self.ax3.set_title('Fourier transform', size=self.configs["fontsize"])
        self.ax3.axis('on')
        self.ax3.set_xlabel('$\omega\ \mathrm{/ \ MHz}$',
                            size=self.configs["fontsize"])
        self.ax3.set_ylim([min(self.Fourier),
                           max(self.Fourier)+0.1*(max(self.Fourier))])
        axval1 = min([self.inputvalues[0], self.inputvalues[2],
                      self.inputvalues[4], self.inputvalues[6],
                      self.inputvalues[8]])
        ...
        self.figure3.subplots_adjust(left=0.12, bottom=0.18, right=0.98,
                                     top=0.92)
        self.canvas3.draw()
        return
    ...
    def change_config(self):
        if self.dialog2.accept:
            ...
            try:
                ...
                str_tm = ("Something went wrong! " +
                          "\nWrong configuration settings.")
                self.War6 = QMessageBox.warning(self, "Error", str_tm,
                                                QMessageBox.Ok)
...
if __name__ == '__main__':
    ...
    splash_screen = QSplashScreen(QPixmap(
                                    resource_path("SimPel_splashicon.png")))
    splash_screen.show()
